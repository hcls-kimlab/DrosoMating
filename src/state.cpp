...
//using namespace cv;
using namespace std;
void clear_queue(queue<cv::Point>& q) {
    queue<cv::Point> empty;
    swap(empty, q);
}
State::State() {
    size2_time = 0;
...
void State::Trajectory(vector<cv::Point> p) {
    float dis1, dis2;
    if (fly1.empty()) {
        if (p.size() == 2) {
            fly1.push(p[0]);
    ...
    if (num >= fps) {
        av_center1.push(cv::Point(average1_x, average1_y));
        av_center2.push(cv::Point(average2_x, average2_y));
        average1_x = 0;
        average1_y = 0;
        average2_x = 0;
        average2_y = 0;
...
void State::update_state(vector<cv::Point> p, int index) {
...
float State::dis(cv::Point p1, cv::Point p2) {
    float disance;
